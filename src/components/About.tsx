...
const About = () => {
  ...
  return (
    <section 
      id="about" 
      ref={sectionRef}
      className="py-24 px-6 md:px-12 bg-portfolio-light-gray relative overflow-hidden"
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-12 items-start">
          ...
          <div className={`transition-all duration-700 delay-200 transform ${sectionInView ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
            <h3 className="text-2xl font-semibold mb-6">My Timeline</h3>
            <div className="relative pl-8 border-l border-primary/20">
              {timelineData.map((item, index) => (
                ...
                >
                  <div className="absolute left-[-8px] w-4 h-4 rounded-full bg-primary border-4 border-white"></div>
      ...
      {/* Background pattern */}
      <div className="absolute inset-0 -z-10 bg-[radial-gradient(circle_at_bottom_left,rgba(193,193,255,0.15),rgba(255,255,255,0))]"></div>
    </section>
  );
};
